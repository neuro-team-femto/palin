...
class AgreementMethod(InternalNoiseExtractor):
    ''' 
    This abstract class subserves all InternalNoiseExtractors that rely on an InverseModel of ideal observers, able to identify internal noise and criteria values to match an observed measure of probability of agreement
    The class calls the InverseModel methods, but does not implement methods to compute prob_agreement and prob_first. 
    ...
    def extract_single_internal_noise(cls,data_df, trial_id = 'trial', stim_id = 'stim', feature_id = 'feature', value_id = 'value', response_id = 'response', **kwargs):
        ...
        if 'inverse_model' not in kwargs:
            raise ValueError('no inverse_model provided for AgreementMethod') 

        prob_agree, prob_first = cls.compute_probabilities(data_df, trial_id, stim_id, feature_id, value_id, response_id, **kwargs)

        internal_noise, criteria = cls.estimate_noise_criteria(prob_agree, prob_first, **kwargs)
    ...
    @classmethod
    def estimate_noise_criteria(cls,prob_agree, prob_first, **kwargs):
        '''
        ...
        inverse_model = kwargs['inverse_model']
        
        internal_noise, criteria = inverse_model.run(prob_agree, prob_first, **kwargs)        
